...
chrome.storage.sync.get(
  ...
  function(options) {
    if (!options.affectTables) {
      return;
    }

    let tables = [];

    if (options.affectTables["Timesheet"]) {
    ...
    if (options.affectTables["TimesheetApproval"]) {
    ...
    requestAnimationFrame(() => {
      tables.forEach(table => formatTable(options, table));
    });
  }
);
...
function formatTable(options, table) {
  if (options.tableOptions["showTimeCodeName"]) {
    table.className += " custom-hide-time-code";
  }

  var rows = table.querySelectorAll(
    "tbody > tr.ListItem, tbody > tr.AltListItem, tbody > tr.MarkRow"
  );
  var headerRow = table.querySelectorAll("thead th");

  rows.forEach(function(row) {
    var cells = row.children,
      workOrderIndex = getIndex(
        headerRow,
        table.querySelector('thead [id*="headerwork_order"]')
      ),
      timeCodeIndex = getIndex(
        headerRow,
        table.querySelector('thead [id*="headertimecode"]')
      ),
      workOrderCell = cells[workOrderIndex],
      timeCodeCell = cells[timeCodeIndex];

    // Show work order name, hide work order code
    if (options.tableOptions["showWorkOrderName"] && workOrderCell) {
      ...
      if (customerString.length) {
        title = title.replace(
          customerRegExp,
          "<span class='custom-customer-name'>" + customerString[0] + "</span>"
        );
      }
    ...
    if (options.tableOptions["showTimeCodeName"] && timeCodeCell) {
    ...
    var hourCellIndexes = getIndexes(
      headerRow,
      table.querySelectorAll("th[id*=headerreg_value]")
    );
  ...
  function hideColumn(type) {
    ...
    switch (type) {
      case "Zoom":
        ...
        cells = Array.from(
          table.querySelectorAll(`button[onclick*="action:${type}"]`)
      ...
      default:
        sumCell = table.querySelector(
          `td[id*="__sumRow_${type.toLowerCase()}"]`
        );
  ...
  if (options.hideCells) {
    ...
    for (let cell in options.hideCells) {
      if (options.hideCells[cell]) {
        hideColumn(cell);
      }
    }
  }
